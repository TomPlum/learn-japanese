...
const mode = new LearnMode(1, "Hiragana", "#fdb40e", "あ", new KanaSettingsBuilder().withHiragana().build(), new LearnSettings());
...
test('Should have the class \'notSelected\' when the passed type does not match the selected prop', () => {
    props.selected = new LearnMode(1, "Katakana", "#fdb40e", "あ", new KanaSettingsBuilder().build(), new LearnSettings());
    const { container } = setup();
