...
test("Passing a class should override the default '.bar' class", () => {
    const { container } = render(
        <SessionProgressBar inProgress={true} quantity={50} remaining={20} className="exampleClass" />
    )
...
each([5, 6, 10]).test(
    "Passing in a streak greater than or equal to 5 should render with the warning variant",
    (streak: number) => {
        const { container } = render(
            <SessionProgressBar inProgress={true} quantity={50} remaining={20} streak={streak} />
        )
