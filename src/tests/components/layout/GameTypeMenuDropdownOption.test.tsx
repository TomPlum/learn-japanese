...
test('Clicking on the option should call the onClick event handler', () => {
    const component = render(<GameTypeMenuDropdownOption type={GameType.KANA} selected={GameType.KANA} onClick={onClickHandler} />);
...
test('The option should be active if the selected prop matches the options type', () => {
    const { container } = render(<GameTypeMenuDropdownOption type={GameType.KANA} selected={GameType.KANA} onClick={onClickHandler} />);
    expect(container.firstChild).toHaveClass('active');
});

test('The option should not be active if the selected prop doesn\'t match the options type', () => {
    const { container } = render(<GameTypeMenuDropdownOption type={GameType.KANA} selected={GameType.NUMBERS} onClick={onClickHandler} />);
    expect(container.firstChild).not.toHaveClass('active');
});

test('Wrapping a child component should append the \'icon\' class to it', () => {
    const component  = render(
        <GameTypeMenuDropdownOption type={GameType.KANA} selected={GameType.NUMBERS} onClick={onClickHandler}>
            <p>Example</p>
