...
it("Should render the first option has the default selected value", () => {
...
it("Should call the onChange event handled with the selected value name when the update call succeeds", async () => {
    ...
    expect(onChangeHandler).toHaveBeenCalledWith('Friends Only');
});
...
it("Clicking the button while a previous selection is updating should not call the service again", async () => {
    ...
    expect(screen.queryByText('Friends Only')).toBeInTheDocument();
});
...
it("Should stop rendering the dropdown menu when blurring the button", async () => {
    ...
    // Expand the menu
    fireEvent.click(component.getByText('Public'));
    const dropdownOption = await screen.findByText('Friends Only');
