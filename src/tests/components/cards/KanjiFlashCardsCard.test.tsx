...
test('Clicking the sync button should call the service method', async () => {
    ...
    fireEvent.click(screen.getByTestId('dashboard-settings-menu-button'));
...
test('Should disable the review button if there are no cards to review', async () => {
    ...
    setup();
    expect(await screen.findByText('Review')).toBeDisabled();
});
...
test('Should enable the review button if there are cards to review', async () => {
    ...
    setup();
    expect(await screen.findByText('Review')).not.toBeDisabled();
});
