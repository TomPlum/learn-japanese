...
test('It should render a user settings link if the user is logged in', () => {
    store.dispatch(setUser(testUser));
    const component = renderReduxConsumer(<ProfileCard onDismiss={onDismissHandler} />);
    expect(component.getByText('User Settings').parentElement).toHaveAttribute('href', "/settings?type=user");
});
