...
each([null, undefined]).test(
    "Should disable the input field when the data property is passed falsy",
    (data: Learnable) => {
        props.data = data
        const { input } = setup()
        expect(input).toBeDisabled()
...
each(["fish", "sea creature"]).test(
    "isCorrect should return true if the answer matches any of the data's meanings",
    (answer: string) => {
        const { input } = setup()
...
each(["sushi", "fsih"]).test(
    "isCorrect should return false if the does not match any of the data's meanings",
    (answer: string) => {
        const { input } = setup()
        fireEvent.change(input, { target: { value: answer } })
