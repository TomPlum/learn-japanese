...
import KanaType from "./KanaType.ts"
import { KanaColumn } from "./KanaColumn.ts"
import each from "jest-each"
import KanaRepository from "../../repository/KanaRepository.ts"
  ...
  describe("Learnable", () => {
    ...
    describe("Hint Message", () => {
      ...
      it("Should return a message about diacriticals for a kana with a diacritical mark", () => {
        ...
        const text = kana.getHint()
        expect(text).toBe(
          "This kana is from the 's' column in the Hiragana syllabary. Also, notice the diacritical mark."
        )
      })
      ...
      it("Should return a combination of relevant messages for a kana that is a diagraph with a diacritical mark", () => {
        ...
        const text = kana.getHint()
        expect(text).toBe(
          "Diagraphs usually drop the 1st kana's 2nd letter when transcribed. Also, notice the diacritical mark."
        )
      })
