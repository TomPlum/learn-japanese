...
export default class KanjiRepository implements Repository<Kanji> {
     ...
     * @return results A collection of matching kanji and the field that it was matched on.
     */
    public async getByFilter(page: number, pageSize: number, search: string, grades: number[], levels: number[], strokes?: number): Promise<KanjiSearchResults> {
        ...
        return RestClient.post<KanjiSearchResponseModel>("/kanji/by-filter", request).then(response => {
            const data = response.data;
