...
const GenkiChapter4 = () => {
  const c4p1: GrammarInfoProps = {
    ...
    body: (
      <div>
        ...
        <p>
          The particle が introduces item X. You can also use あります to say that you have or own something or that and
  ...
  const c4p5: GrammarInfoProps = {
    ...
    body: (
      <div>
        <p>
          も is not only used to refer to the second item which shares a common attribute with the first, but it can
          also be used when two or more people perform the same activity. Or when someone buys, sees, or eats two or
          more things or when you go to two places, do something on two different occasions, and so forth. It is
          important to note that も replaces the particles は、が、or を.
        </p>
  ...
  const c4p6: GrammarInfoProps = {
    ...
    body: (
      <div>
        <p>
          The duration of an activity is expressed with a bare noun, like いちじかん. Such nouns are not followed by
          particles and usually appear immediately before the verb. Approximate measurements can be expressed using
          ぐらい after ～じかん.
        </p>
  ...
  const c4p7: GrammarInfoProps = {
    ...
    body: (
      <div>
        <p>
          If you want to use an expression of quantity in Japanese, the quantity word (like たくさん) can either go
          before the noun or after the particle を.
        </p>
