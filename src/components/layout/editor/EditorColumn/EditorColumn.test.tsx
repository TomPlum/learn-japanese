...
test("Should render a card when dropping with valid details", () => {
  ...
  const { component } = render(<EditorColumn />)

  const dropEvent = createEvent.drop(component.container.firstChild!, { dataTransfer: { getData: mockGetData } })
  fireEvent(component.container.firstChild!, dropEvent)
...
test("Should add the dropping class to the container when dragging over the column", () => {
  const { component } = render(<EditorColumn />)
  const wrappingDiv = component.container.firstChild!
