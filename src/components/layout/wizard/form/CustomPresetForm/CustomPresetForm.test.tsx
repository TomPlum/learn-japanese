...
test("Clicking save when the form is valid should save the preset", async () => {
  ...
  // Should then display success message
...
test("Should display the a default error message if the save preset API call fails", async () => {
  // Assume the service call will be un-successful
  server.use(...useSavePlayPresetHandlersError)
  const { name, save } = setup()
