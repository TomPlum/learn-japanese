import MemoryGame, { MemoryGameProps }  from "./MemoryGame"
...
test("Passing data settings with 0 question quantity should set 1 current question value", async () => {
  ...
  props.settings = new GameSettingsBuilder()
    .withQuestionSettings(new QuestionSettingsBuilder().withQuantity(0).build())
    .build()
  setup()
...
test("Answering all questions correctly should stop the timer", () => {
  props.settings = new GameSettingsBuilder()
    .withTimeSettings(new TimeSettingsBuilder().isTimed().isCountDown(false).build())
    .build()
...
test("Failing to correctly answer the question before the countdown finishes should present the next kana", async () => {
  props.settings = new GameSettingsBuilder()
    .fromExisting(props.settings)
...
test("Failing to correctly answer the question before the countdown finishes should subtract a life if enabled", async () => {
  props.settings = new GameSettingsBuilder()
    .fromExisting(props.settings)
    .withTimeSettings(new TimeSettingsBuilder().isCountDown().withSecondsPerQuestion(5).build())
    .withLifeSettings(new LifeSettingsBuilder().isEnabled(true).withQuantity(10).build())
...
test("Setting the game settings score property to true should render the score", () => {
  props.settings = new GameSettingsBuilder()
    .fromExisting(props.settings)
    .withQuestionSettings(
      new QuestionSettingsBuilder().withType(QuestionType.CHOICE).withCardQuantity(2).withScoreTracking(true).build()
    )
    .build()
  setup()
...
test("Setting the game settings score property to false should not render the score", () => {
  props.settings = new GameSettingsBuilder()
    .fromExisting(props.settings)
    .withQuestionSettings(
      new QuestionSettingsBuilder().withType(QuestionType.CHOICE).withCardQuantity(2).withScoreTracking(false).build()
    )
    .build()
  setup()
...
test("Clicking the quit button should render the confirmation modal", () => {
  ...
  expect(
    screen.getByText("You'll lose your current progress, but you'll see the results of your game thus far.")
  ).toBeInTheDocument()
})
